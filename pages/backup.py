...
def backup():
    ...
    # Function to handle owned/backup switch toggle
    def on_switch_toggle(value):
        if value:
            # Filter for owned contracts
            filtered = [row for row in contract_rows if row['role'] == 'owned']
            ui.notify("Showing owned contracts", type="info")
        else:
            # Filter for backup contracts
            filtered = [row for row in contract_rows if row['role'] == 'backup']
            ui.notify("Showing backup contracts", type="info")
